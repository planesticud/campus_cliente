...
export class ListTipoDescuentoMatriculaComponent implements OnInit {
  ...
  loadData(): void {
    this.descuentosPosgradoService.get('tipo_descuento_matricula/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.descuentosPosgradoService.delete('tipo_descuento_matricula/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'TipoDescuentoMatricula deleted');
            }
         });
      }
    });
